...
export const updateLivePatch = defineMethodV2('updateLivePatch', {
  meta: {
    description:
      'Check for and apply the latest verified live patch to this session',
    action: 'update',
    namespace: 'session',
    license: 'none',
    functionality: 'both',
  ...
  input: z.object({
    trigger: z.enum(['client', 'api', 'keyboard']).default('api'),
  }),
  parameterOrder: ['trigger'],
  output: z.object({
    updated: z.boolean(),
    status: z.enum(['up_to_date', 'updated', 'rolled_back', 'failed']),
    oldHash: z.string().nullable(),
    newHash: z.string().nullable(),
    reloadDurationMs: z.number().nullable(),
