...
export interface ClientFunctionMetadata {
  ...
  availableDuringRuntimeMutation?: boolean;
...
function getNamespaceNouns(namespace?: string): string[] {
  ...
  return (
    namespaceNounMap[namespace] ?? [
      namespace.charAt(0).toUpperCase() + namespace.slice(1),
    ]
  );
}

export function deriveNamespacedMethodName(
  functionName: string,
  namespace?: string,
): string {
  ...
  const nouns = getNamespaceNouns(namespace).sort(
...
function getResolvedNamespacedName(meta: ClientFunctionMetadata): string {
  return (
    meta.aliases?.namespacedName ??
    deriveNamespacedMethodName(meta.functionName, meta.namespace)
  );
}

function getFlatAliases(meta: ClientFunctionMetadata): {
  aliases: string[];
  deprecated: string[];
} {
...
export function getMethodAliases(meta: ClientFunctionMetadata): string[] {
  ...
  const defaultNamespacedName = deriveNamespacedMethodName(
    meta.functionName,
    namespace,
  );
...
export function getParameterMetadata(
  ...
  return (
    parameterRegistry.get(schema) ?? parameterRegistry.get(unwrapSchema(schema))
  );
}

export function buildKeyAliasMap(
...
>(
  ...
  [
    Shape[Keys[0]],
    ...{ [K in keyof Keys]: K extends 0 ? never : Shape[Keys[K]] }[number][],
  ]
> {
...
>(
  ...
  params: {
    meta: Omit<
      ...
      'functionName' | 'parameterOrder' | 'inputSchema' | 'outputSchema'
    >;
    input: T;
  ...
  const tupleSchema = zObjectToTuple(
    params.input,
    params.parameterOrder as any,
  );

  const inputUnion = z.union([z.tuple([params.input]), tupleSchema]);
...
export type InferInput<T extends CapabilityDefinition> = z.infer<
  T['inputSchema']
>;
export type InferOutput<T extends CapabilityDefinition> = z.infer<
  T['outputSchema']
>;
