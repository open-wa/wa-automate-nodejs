...
function routePathForMethodName(
  basePath: string,
...
export function getHttpMethodDefinitions(
  basePath = '/api',
  ...
  return clientRegistry.getAll().map((def) => {
    ...
    const aliasNames = uniqueStrings([
      ...
      ...aliases,
    ]).filter((alias) => alias !== `${namespace}.${namespacedName}`);
    ...
    return {
      ...
      availableDuringRuntimeMutation: def.meta.availableDuringRuntimeMutation,
    };
  });
}
