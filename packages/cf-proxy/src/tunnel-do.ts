import { AnyProxyMessage } from './types';

export class SessionTunnel implements DurableObject {
  private pendingRequests = new Map<string, { resolve: (res: Response) => void, timeout: ReturnType<typeof setTimeout> }>();
  ...
  async handleHttpAsRpc(request: Request, subpath: string): Promise<Response> {
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer) {
    ...
    let msg: AnyProxyMessage;
    try {
      msg = JSON.parse(message) as AnyProxyMessage;
    } catch {
      return;
    }
