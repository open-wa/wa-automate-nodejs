...
function createBootstrapFailure(error: unknown): Error {
  ...
  return new Error(`Lightpanda bootstrap failed before QR milestone: ${message}`);
}

async function waitForLightpandaBootstrapMilestone(client: OpenWAClient, timeoutMs = 15_000): Promise<{
  pageUrl: string;
  phase: string;
}> {
...
async function cleanupClient(client: OpenWAClient): Promise<void> {
  void client.stop('lightpanda_smoke_cleanup').catch(() => undefined);
  await Promise.race([
