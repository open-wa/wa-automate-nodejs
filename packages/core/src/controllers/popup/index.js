...
const setUpApp = async () => {
    express = (await Promise.resolve().then(() => __importStar(require('express')))).default;
    app = express();
...
async function popup(config) {
    ...
    const popupListener = events_1.ev.on('**', async (data, sessionId, namespace) => {
        ...
        if (data?.includes && data?.includes("ready for account")) {
            popupListener.off();
            await (0, exports.closeHttp)();
        }
    }, { objectify: true });
    if (server)
        return `http://localhost:${PORT}`;
    PORT = await (0, get_port_1.default)({ host: 'localhost', port: typeof preferredPort == 'number' ? [preferredPort, 7000, 7001, 7002] : [7000, 7001, 7002] });
    ...
    const hasChrome = await (0, command_exists_1.default)(appName).then(() => true).catch(() => false);
    if (hasChrome) {
        if (!config?.inDocker)
            await (0, open_1.default)(`http://localhost:${PORT}${config?.qrPopUpOnly ? `/qr` : ``}`, { app: {
