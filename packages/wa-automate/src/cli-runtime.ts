...
function bridgeRuntimeEvents(
  ...
  if (
    typeof openwaClient.events.onAny === 'function' &&
    typeof openwaClient.events.offAny === 'function'
  ) {
  ...
  const bridgedEvents = new Set(
    eventRegistry.getAll().map((def) => def.meta.eventName),
  );
  ...
  bridgedEvents.forEach((eventName) => {
    ...
    unsubscribers.push(() =>
      openwaClient.events.off(eventName as never, handler),
    );
  });
...
function attachLaunchNarration(
  ...
  on('launch.helper.pre_api.before', (event: any) => {
    sink.status({
      ...
      sessionId,
      detail:
        event?.details?.mode === 'scripts'
          ? 'Injecting pre-API helper assets...'
          : 'Pre-API helpers already present',
    });
  });
  ...
  on('internal_launch_progress', (event: any) => {
    ...
    if (event?.text)
      sink.write({ level: 'info', message: `${prefix}${event.text}`.trim() });
  });
  ...
  on('launch.patch.init.after', (event: any) => {
    if (event?.details?.phase === 'preload') {
      ...
      const available = Array.isArray(event?.details?.available)
        ? event.details.available.length
        : 0;
  ...
  on('patch.init.before', (event: any) => {
    if (event?.details?.phase === 'apply') {
      const count = Array.isArray(event?.details?.patchIds)
        ? event.details.patchIds.length
        : 0;
      sink.status({
        phase: 'launch.patch',
        sessionId,
  ...
  on('patch.init.after', (event: any) => {
    if (event?.details?.phase === 'apply') {
      const applied = Array.isArray(event?.details?.applied)
        ? event.details.applied
        : [];
...
const CHROME_CACHE_FILE = resolve(
  process.cwd(),
  '.open-wa',
  'chrome-executable-path.json',
);
...
export function readChromePathCache(
  cacheFilePath: string = getChromeCacheFilePath(),
): ChromePathCacheRecord | undefined {
  ...
  try {
    ...
    return {
      executablePath:
        typeof raw.executablePath === 'string' ? raw.executablePath : undefined,
...
export function writeChromePathCache(
  executablePath: string,
  cacheFilePath: string = getChromeCacheFilePath(),
): void {
  ...
  writeFileSync(
    cacheFilePath,
    JSON.stringify(
      { executablePath, updatedAt: new Date().toISOString() },
      null,
      2,
    ),
    'utf8',
...
export function clearChromePathCache(
  cacheFilePath: string = getChromeCacheFilePath(),
): void {
  if (existsSync(cacheFilePath)) {
...
export function isUsableExecutablePath(
  executablePath?: string,
): executablePath is string {
  return !!executablePath && existsSync(executablePath);
}

function getExplicitUseChromePreference(
...
export function parseCliArgs(
  ...
  const apiKey =
    getVal(argv, '--api-key') || getVal(argv, '--key') || getVal(argv, '-k');
  ...
  if (argv.includes('--aggressive-garbage-collection'))
    cliOverrides.aggressiveGarbageCollection = true;
  if (argv.includes('--no-dashboard')) cliOverrides.dashboard = false;
  if (argv.includes('--ephemeral')) cliOverrides.ephemeral = true;

  if (argv.includes('--live-patch')) cliOverrides.livePatch = true;
  const pollPatch = getOptionalVal(argv, '--poll-patch');
  if (pollPatch !== undefined) {
    cliOverrides.pollPatch =
      pollPatch === true
        ? true
        : /^\d+(?:\.\d+)?$/.test(pollPatch)
          ? Number(pollPatch)
  ...
  if (cliOverrides.livePatch && cliOverrides.pollPatch !== undefined) {
    throw new Error('--live-patch and --poll-patch are mutually exclusive');
  }

  const livePatchEndpoint = getVal(argv, '--live-patch-endpoint');
  if (livePatchEndpoint) cliOverrides.livePatchEndpoint = livePatchEndpoint;
  ...
  if (readyWebhook)
    unsupportedWarnings.push(
      `--ready-webhook was provided (${readyWebhook}) but ready-webhook delivery is not yet wired in v5 CLI.`,
    );
  ...
  if (argv.includes('--stats')) {
    unsupportedWarnings.push(
      '--stats was provided but swagger-stats parity is not restored; v5 only exposes the compatibility redirect/deprecation route.',
    );
  }

  if (argv.includes('--generate-api-docs')) {
    unsupportedWarnings.push(
      '--generate-api-docs was provided; runtime docs are always served from the shared API layer and no separate collection generation step currently runs at CLI boot.',
    );
  }

  if (argv.includes('--tunnel')) {
    unsupportedWarnings.push(
      '--tunnel was provided but tunnel setup parity is not yet restored in the v5 CLI.',
    );
  }

  if (
    argv.includes('--chatwoot-url') ||
    argv.includes('--twilio-webhook') ||
    argv.includes('--bot-press-url')
  ) {
    unsupportedWarnings.push(
      'Legacy integration flags (Chatwoot/Twilio/BotPress) were provided but are not yet wired into the v5 CLI boot path.',
    );
  }
...
export async function resolveExecutablePath(
  ...
  const detectedPath = Launcher.getInstallations().find((installationPath) =>
    isUsableExecutablePath(installationPath),
  );
  ...
  return {
    source: 'driver_default',
    warning:
      'Chrome resolution warning: no valid local Chrome installation was found. Falling back to Puppeteer/default driver browser resolution.',
  };
}

function resolveLightpandaExecutablePath(
  config: Config,
...
async function selectRuntimeDriver(
  config: Config,
  rawConfigs?: TrackedConfig['rawConfigs'],
...
function printStartupSummary(
  ...
  const host = config.host.includes('http')
    ? config.host
    : `http://${config.host}`;
  ...
  if (resolution.source === 'lightpanda_config' && resolution.executablePath) {
    ...
    sink.write({
      level: 'info',
      message:
        'Browser executable: Lightpanda SDK managed executable (shared cache/default resolution)',
    });
    ...
    sink.write({
      level: 'info',
      message:
        'Browser executable: driver default fallback (local Chrome unavailable)',
    });
  }
  if (config.webhook) {
    sink.write({
...
export async function start(
  parsedArgs: ParsedCliArgs = parseCliArgs(),
  ...
  unsupportedWarnings.forEach((warning) =>
    sink.write({ level: 'warn', message: `Compatibility warning: ${warning}` }),
  );

  const driverSelection = await selectRuntimeDriver(config, rawConfigs);
  const { driver, engineLabel, executableResolution, preferLocalChrome } =
    driverSelection;
  ...
  const openwaClient = await createClient({
    ...
    qrTimeoutMs:
      typeof config.qrTimeout === 'number'
        ? config.qrTimeout * 1000
        : undefined,
    authTimeoutMs:
      typeof config.authTimeout === 'number'
        ? config.authTimeout * 1000
        : undefined,
    ...
    patchConfig: {
      ghPatch: config.ghPatch,
      cachedPatch: config.cachedPatch,
    },
    livePatchConfig: {
      livePatch: config.livePatch,
      pollPatch: config.pollPatch,
      endpoint: config.livePatchEndpoint,
      publicKey: config.livePatchPublicKey,
    },
  });
  ...
  const detachLaunchNarration = attachLaunchNarration(
    openwaClient,
    sink,
    config.sessionId,
  );
  ...
  const detachRuntimeBridge = bridgeRuntimeEvents(
    openwaClient,
    publishRuntimeEvent,
  );
  ...
  try {
    ...
    sink.write({
      level: 'warn',
      message:
        'The server is running — use /health and /api-docs to inspect state.',
    });
...
export async function main(
  ...
  if (parsedArgs.pm2) {
    ...
    try {
      ...
      const pm2Flags = parsedArgs.forwardedArgs.filter(
        (flag: string) => flag !== '--pm2',
      );
      const cliPath = fileURLToPath(
        new URL('../dist/cli.cjs', import.meta.url),
      );
