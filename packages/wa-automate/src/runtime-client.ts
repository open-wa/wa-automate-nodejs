...
function readChromePathCache(
  ...
  try {
    ...
    const raw = JSON.parse(readFileSync(cacheFilePath, 'utf8')) as {
      executablePath?: string;
    };
...
export async function selectRuntimeDriver(
  ...
  const preferLocalChrome = shouldPreferLocalChrome(
    config,
    options.rawConfigs,
    options.defaultPreferLocalChrome ?? true,
  );
