...
| `disconnected` | The connection dropped after startup. This can be temporary, or it can mean the account was logged out. |
| `stopping` | The process is shutting the client down on purpose. Do not treat this as a logout. |
| `stopped` | The client has finished shutting down. A new process start is necessary before it can connect again. |
...
Log all other transitions. A valid transition can still occur, but the log entry must include the raw state, previous state, reason, and session ID.
...
4. If the state is `disconnected`, wait for a short backoff period. Permit one reconnect attempt from the same saved session.
...
7. If reconnect attempts occur again, keep the session offline. Do a manual check before you send more messages.

Keep the recovery loop small. Use one immediate reconnect window and one delayed restart. Then, send an alert if the session cannot get to `ready`.
