...
```bash
npx @open-wa/wa-automate@alpha --port 8080 --log-console --verbose > open-wa.log 2>&1
```
...
3. If you use SocketClient, pass the key as the second argument to `SocketClient.connect()`.
4. Replace the key if it was copied into logs, browser code, or shared chat.
