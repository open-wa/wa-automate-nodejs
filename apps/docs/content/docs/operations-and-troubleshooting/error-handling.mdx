...
These specific errors, especially as a crash loop shortly after the page
loads, almost always mean one of:

1. **`--single-process` or `--no-zygote` in `chromiumArgs`.** These crash
   modern Chrome on WhatsApp Web. The runtime strips them by default and logs
   `Removed browser args known to crash WhatsApp Web`; if you forced them with
   `allowDangerousBrowserArgs: true`, remove that. This is the most common
   cause in low-RAM Docker/Railway deployments.
2. **The renderer was OOM-killed.** Check container memory limits and host
   `dmesg` / container OOM events. See the low-memory guidance on the Docker
   page.
3. **Too-small `/dev/shm`.** Give the container real shared memory
   (`--shm-size=1gb`) or keep `--disable-dev-shm-usage`.

Do not respond to detached-frame errors by adding more Chromium flags — the fix
is almost always removing flags and giving the browser enough memory.
...
<Accordion title="Session will not authenticate" id="session-will-not-authenticate">
...
<Accordion title="Browser will not launch" id="browser-will-not-launch">
...
<Accordion title="Easy API returns 401" id="easy-api-returns-401">
...
<Accordion title="Easy API returns 429" id="easy-api-returns-429">
...
<Accordion title="Account looks restricted" id="account-looks-restricted">
