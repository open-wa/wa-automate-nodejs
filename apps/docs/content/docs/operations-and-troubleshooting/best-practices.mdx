...
WhatsApp does not publish fixed automation limits. Production systems must expect limit changes without notice. Send messages slowly. Do not send identical bulk messages. Add backoff after failures. Prefer automation that replies to received messages.
...
- respond only to messages that must start the agent
