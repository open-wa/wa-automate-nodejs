...
This page explains the processes that operate before your code can send messages or receive events. It also explains which process owns the browser session. Use this information to select Easy API, custom code, or a remote consumer.
...
<Step title="Attach the runtime bridge">
    open-wa attaches the automation/runtime bridge.
</Step>
<Step title="Expose the client surface">
...
Injection is treated as part of session readiness, not just a setup detail. The runtime checks that the bridge, store, and necessary methods are present before the client is considered ready. If a navigation or reload removes the bridge, the runtime can detect that and run a reinjection path before exposing the session again.
...
3. **authenticated**: WhatsApp Web has accepted the login, but the runtime can still be syncing, injecting, or validating
...
Modern session persistence uses a browser profile. Unless you select `ephemeral`, open-wa resolves a `userDataDir` for the browser profile. If you do not provide one, the runtime derives a directory from the session ID. It usually uses `./_IGNORE_<sessionId>` relative to `sessionDataPath` or the working directory.
...
The CLI can also create `.open-wa/chrome-executable-path.json` to cache the resolved Chrome executable path. That file is runtime convenience data, not WhatsApp authentication state.
...
- how do remote consumers connect to it?
...
- Use **Remote-consumer mode** when another service must communicate with an Easy API process that owns the browser session.

For each path, make sure that the session gets to **ready** before you send commands. Keep session data in durable storage when restarts are necessary. Protect all remote surfaces with API keys or proxy tokens.
