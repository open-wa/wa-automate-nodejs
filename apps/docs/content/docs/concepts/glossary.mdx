...
Re-auth is the process of bringing an existing session back online without a fresh QR scan when the necessary auth state is still usable.
...
Session data is the authentication material that restores a session. Its portability depends on the auth/runtime mode that you use.
...
A tunnel, such as Cloudflare Tunnel, exposes a local service to the internet. It does not open ports on your machine or network.
