...
description: API reference for the primary Open-WA Client and startup options.
---
...
| `OpenWAClient` | The object returned by `createClient`. | You created a core runtime and must start it, stop it, inspect readiness, access events, or connect to the transport. |
...
For most applications, start with Easy API. Use `createClient` when you must own the runtime. Examples include a specified driver, custom plugin registration, direct readiness checks, and controlled shutdown behavior.

## Runnable example

This example starts the core runtime with the Puppeteer driver. It listens for lifecycle and message events and prints the readiness snapshot. Process signals cause a controlled shutdown.
...
The first run needs authentication. After the session reaches `READY`, `getReadiness()` reports `ready: true` and `exposureSafe: true`.
...
| `client.start()` | Initializes the driver and opens WhatsApp Web. It completes authentication, validation, finalization, and readiness events. |
...
Readiness is stricter than state. Authentication can finish while runtime validation, patch handling, license handling, finalization, or transport checks remain. Examine `getReadiness().ready` first. Then, examine `pending`, `blockers`, and `requirements` to find why a session is not ready.
...
The primary client interface returned by `createClient`.

<AutoTypeTable path="../../../../../packages/core/src/createClient.ts" name="OpenWAClient" />

## Plugin type references

Core re-exports the plugin SDK types for backward compatibility. New plugins must import these types from `@open-wa/plugin-sdk`. Core also exposes them through `@open-wa/core/plugins` for internal consumers and old integrations.
...
Plugin authors must import from `@open-wa/plugin-sdk`. Core uses the same contracts when it registers plugins and makes the filtered plugin runtime.
