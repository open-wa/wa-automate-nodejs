...
> `apps/docs/scripts/gen-workspace-readme-docs.js` generated this page from `packages/orchestrator/README.md`. Do not edit this page directly.
...
You can explore all possible API requests for the relevant sessions at `/api-docs/`. Open this path in a web browser:
...
Then go to the home directory. The `/root` directory can cause more problems.
...
The command can start the session. If Puppeteer still causes problems, try this command:
...
If the installation succeeded, run the CLI again:
