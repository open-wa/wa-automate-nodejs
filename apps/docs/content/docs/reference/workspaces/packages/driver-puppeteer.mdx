---
title: "@open-wa/driver-puppeteer"
description: "Puppeteer driver implementation for open-wa"
---

> `apps/docs/scripts/gen-workspace-readme-docs.js` generated this page from `packages/driver-puppeteer/README.md`. Do not edit this page directly.

# @open-wa/driver-puppeteer

Puppeteer driver implementation for open-wa

Part of the [@open-wa v5 monorepo](https://github.com/open-wa/wa-automate-nodejs).

## Install

```bash
pnpm add @open-wa/driver-puppeteer
```

## Documentation

See the [docs site](https://openwa.dev).

## License

[H-DNH V1.0](https://github.com/open-wa/wa-automate-nodejs/blob/main/LICENSE.md) — Hippocratic + Do Not Harm
