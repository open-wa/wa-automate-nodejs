...
description: Deploy a reverse-tunnel proxy on Cloudflare Workers to access your sessions remotely, no port forwarding necessary.
---
...
Replace a token if users share it too widely, it appears in logs, or a consumer no longer needs access.
...
Then, update the remote clients to use the new token. Close and reconnect the consumer WebSocket connections. The connections will authenticate with the new value.
