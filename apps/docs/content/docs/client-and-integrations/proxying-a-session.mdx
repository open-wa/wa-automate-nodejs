...
Use a **SOCKS** or **SOCKS5** proxy for a lower-level tunnel endpoint. Also use it when an HTTP proxy causes WebSocket or browser network failures.

If the two proxy types are available, start with `http://` or `https://`. Switch to `socks5://` if the HTTP proxy fails.
...
In production, put the proxy in your session configuration. Make sure that the runtime host can connect to the proxy before you start a long-lived session. Use separate proxy credentials for each environment.
...
Do not put proxy credentials in the proxy URL. Keep the address, username, and password separate. This structure makes logs safer and prevents URL parsing problems with special characters.
...
The returned IP must match the proxy exit IP, not the normal public IP of the server.
...
- Keep proxy credentials in environment variables or a protected secret store.
