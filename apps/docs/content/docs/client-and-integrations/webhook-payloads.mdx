...
<Callout type="note" title="This complements webhook usage docs">
  Use the Easy API guide to run the sender. Use Webhooks for Business for a receiver procedure.
</Callout>

## Enablement flow

Use the source-backed v5 webhook integration plugin from config. The CLI still accepts `--webhook`, but it does not register webhook delivery. For this alpha release, use config for reliable event delivery:
...
Every webhook delivery is a JSON `POST`. Your endpoint must authenticate the request and process only the applicable events. During development, log the raw body. Then, return a `2xx` response.
...
The local `curl` request returns `HTTP/1.1 204 No Content`. A `401` means that the shared secret header did not match. A malformed envelope returns `400`. Open-wa identifies a non-`2xx` response as a failed delivery and can try the delivery again. Correct receiver validation before you send production events.
...
The current webhook plugin does not generate a request signature. For HMAC verification, add a signing proxy in front of your receiver. Alternatively, sign the request in your infrastructure. Verify the raw body before you parse it.
...
After the local test returns `204`, configure the receiver URL. Send one WhatsApp message. Compare the raw log with the envelope fields above.

## Delivery guarantees

Webhook delivery is **at least once**. Your endpoint can receive the same event more than one time. This can occur after a response timeout or incomplete processing.
...
| `payload.message.body` | The text body. Media messages can have an empty body and use `caption` instead. |
