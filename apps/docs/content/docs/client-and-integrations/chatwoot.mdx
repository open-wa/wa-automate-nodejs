...
If you are reading older Easy API examples, you can see these flags:
...
The plugin path is the recommended path for new deployments. It loads `@open-wa/integration-chatwoot`, validates `pluginConfig.chatwoot`, mounts the Chatwoot webhook route, and handles WhatsApp to Chatwoot and Chatwoot to WhatsApp sync inside the plugin host.
...
When the integration works, Chatwoot shows an inbox conversation with the WhatsApp sender as the contact. Incoming WhatsApp messages appear as incoming Chatwoot messages. Agent replies appear as outgoing messages. Media appears as attachments or file links, based on the media source.
...
- **Chatwoot replies do not arrive in WhatsApp**: Make sure that Chatwoot can access `https://wa.example.com/plugins/chatwoot/webhook?api_key=your-secure-key`. Also make sure that the `apiKey` value matches the open-wa config.
...
1. protect the inbound webhook path
2. confirm incoming and outgoing message sync
