---
title: Socket Client
description: Connect your app to a remote Easy API instance with SocketClient.
---
...
The SSE event stream adds the same key to the events URL as `api_key`. The standard `EventSource` API does not support custom headers.
...
In most cases, listen for `disconnect` and keep your consumer idempotent. A stopped event stream does not always mean that the WhatsApp session stopped. Examine the Easy API session health before you restart the runtime.
...
Older v4 deployments and older socket.io-based examples can not match this transport. If you are running the stable v4 line, use the SocketClient version and docs that match that deployment. If you are testing v5 alpha, keep Easy API and `@open-wa/socket-client` on matching current packages from this repo.

## Browser security

`SocketClient` does not give your app browser access. It only gives your app API access to a running Easy API instance.

Your consumer can call exposed Easy API methods and receive events. It cannot inspect the browser page or run browser-side code. It cannot bypass the Easy API security boundary. Treat the API key as a server secret.

## Plugin SocketClient guidance

Plugins outside the Easy API process can use `SocketClient` to connect to a local Easy API instance. Use this design when a separate worker, sidecar, or integration service must not own the WhatsApp browser runtime.
...
- Verify that the base URL includes the protocol and the correct port.
