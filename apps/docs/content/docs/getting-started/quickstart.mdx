...
- **Node.js >= 22.21.1**, this is necessary. The library will not work on older versions. Check with `node --version`.
- **npm or pnpm**, any package manager works, but the repository uses pnpm for development.
- **A WhatsApp account** on a linked phone, this automates WhatsApp Web, not the mobile app.

<Callout type="warning">
**v5 is an alpha release.** Test it in a disposable environment before production use. For production systems, use v4: `npx @open-wa/wa-automate@4.76.0`
</Callout>
...
If you know the host account number, use **[Link-code login](/docs/getting-started/link-code)**. This flow does not use a QR scan.
...
Your phone receives the message within seconds.

Expected API result, illustrative and abbreviated. The Easy API puts the `sendText` message ID in `data`:
