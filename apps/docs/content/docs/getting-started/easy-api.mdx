...
Exact log lines vary by version and browser driver. The runtime starts and then prints the local API address and docs URL.
...
Use `--api-key "your-secure-key"` when your apps, scripts, or team members need a stable key. The CLI reads the value after `--api-key`, `--key`, or `-k`. A bare `--api-key` does not generate a key.
...
- If you know the host account number and do not want to scan a QR code, use **[Link-code login](/docs/getting-started/link-code)**.
...
- Configure a restart policy before you use the process in production.

## Production warning

<Callout type="warning">
Do not expose Easy API to the public internet without a clear security boundary. Use an API key and limit network access. Use HTTPS behind a trusted proxy. Protect session profile data. Do not share interactive docs with untrusted users.
</Callout>

## Tunneling and remote access

The current v5 CLI only shows a warning when you use `--tunnel`. It does not configure a tunnel. Use **[Cloudflare Session Proxy](/docs/client-and-integrations/cf-proxy)** for controlled remote access.
...
```bash
npx @open-wa/wa-automate@alpha --pm2 --session-id STICKER-BOT --port 8080
```
