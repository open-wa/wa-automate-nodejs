...
On the first run, authenticate the session. Scan the QR code or use another supported login flow. When the session is ready, send `Hi` to the connected account. The account sends the reply above.
...
On the first run, the runtime must authenticate the WhatsApp session. Scan a QR code or use another supported login flow. The available flow depends on your config and environment.

After the first successful login, the runtime saves session data for the configured `sessionId`. Later runs with the same session can restore that state. The process needs valid saved auth data. If the data is available, you do not need to log in again.
...
<Callout type="tip" title="Default to createClient">
  v5 is the forward path, but some examples still show the older `create()` shape for compatibility. Treat `createClient` as the default for new v5 custom code unless a page says otherwise.
</Callout>

## Plugin vs custom code

Use plugins to make reusable integrations. You can load a plugin through config, share it across projects, or publish it as a package.
...
- If you use your Chrome installation, specify it in the config. Do not depend on automatic discovery.
