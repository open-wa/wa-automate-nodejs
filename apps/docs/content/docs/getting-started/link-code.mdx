...
Yes. The QR code can still appear as a fallback during authentication.

If link-code login fails or times out, scan the QR code. This alternative works while the runtime continues to wait for authentication.
...
- Make sure that you use the WhatsApp account that owns the number in `linkCode`.
...
- Use this only when you know which account will own the session.
