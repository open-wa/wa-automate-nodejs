...
A valid session token lets the browser reconnect without another scan. This works if the phone did not log out the session.

## Session Storage

### Where Session Data Lives

Open-wa stores session data on disk in a session directory. By default, it creates this directory in the CLI working directory.
...
3. The runtime saves the session token to disk
4. The API becomes ready for use

### Timeout Behavior

If no one scans the QR code before the `qrTimeout`, the runtime emits a new code. This counts toward the `maxQr` limit.
...
- If the account is already linked to too many devices, linking can be rejected

### Does QR Still Appear?

Link-code mode bypasses the QR code flow. If link-code authentication fails, the runtime can show a QR code.
...
- You do not need to scan again unless WhatsApp invalidated the session.
...
- If session files are corrupted, you can need to delete them and re-authenticate
