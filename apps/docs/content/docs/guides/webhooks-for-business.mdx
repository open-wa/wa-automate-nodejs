...
description: Connect WhatsApp events to your backend with webhooks. This guide has source-backed v5 payload examples.
---
...
The source-backed v5 alpha path loads the webhook integration plugin from config. The CLI still parses `--webhook`, but it does not register webhook delivery. Do not use that flag for delivery in this alpha release.
...
| Message id | `payload.message.id` | Deduplicate retries and do not process the same message two times. |

<Callout type="warning" title="Do not mix envelope shapes">
Older snippets can show `{ event, data }`. The current v5 webhook plugin source sends `{ webhookId, sessionId, event, payload, timestamp }`. Use `payload`, not `data`, for v5 webhook receivers.
</Callout>
...
The local `curl` request returns `HTTP/1.1 204 No Content`. If your receiver returns `401`, correct the shared secret header. Open-wa identifies all other non-`2xx` responses as failed deliveries. It tries these deliveries again with the reliability rules below. After the test passes, configure the receiver URL and send one WhatsApp message. Make sure that the production path receives it.

## Platform mappings

<Tabs items={['Zapier', 'Make.com', 'n8n']}>
<Tab value="Zapier">
