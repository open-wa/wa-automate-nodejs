...
You can run multiple sessions in one process. Each session needs a unique `sessionId`. You must manage each session separately.
...
```bash
npx @open-wa/wa-automate@alpha --session-id sales --port 8081 --api-key "sales-api-key"
npx @open-wa/wa-automate@alpha --session-id support --port 8082 --api-key "support-api-key"
npx @open-wa/wa-automate@alpha --session-id ops --port 8083 --api-key "ops-api-key"
```
...
This keeps routing inside your app, but one process owns every browser instance. If that process exits, every session exits with it.
...
Keep a small record that shows the location of each session. Use unique ports and API keys for each Easy API process.
...
Do not share one API key across all sessions unless your network boundary already separates tenants. Separate keys make cross-session calls easier to find. You can replace one session key without changing the other sessions.

## Persistence isolation

Each session must have separate session data. The `sessionId` prevents the runtime from mixing data from different accounts.
...
Prevent data transfer between sessions. A defective auth state for `support` must not change `sales`. When you delete one session, the login state of another session must remain.
...
For webhook delivery in the current v5 alpha, configure `@open-wa/integration-webhook` in each session config. The CLI still parses `--webhook`, but it does not register webhook delivery. Do not use that flag for delivery.
...
One simple pattern is to put `sessionId` in the URL your app exposes, then map
it to the Easy API method route, which is always prefixed with `/api/`:

```text
POST /sessions/sales/api/sendText   ->  http://localhost:8081/api/sendText
POST /sessions/support/api/sendText ->  http://localhost:8082/api/sendText
```

Then proxy the call to the matching Easy API process (note the `/api/` prefix
on the Easy API path):
...
Keep authorization in your app. Permission to send from `sales` must not give a user permission to send from `support`.
...
If one session stops in a process-per-session setup, the failure must affect only that session. Other sessions keep their browser process, port, API key, and event stream.
...
- alert when a port stops responding, QR login is necessary again, or message processing stalls
...
After a restart, the session uses its persistent data again. A valid auth state lets the session reconnect without changes to other sessions. If the account needs a new QR scan, handle its QR flow separately. Keep the other sessions in operation.
