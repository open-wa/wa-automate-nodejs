...
- **Old messages**: Deletion can fail for messages older than the allowed window

The exact time windows are controlled by WhatsApp and can change without notice.
...
- Deletion is not guaranteed. WhatsApp can reject deletion requests.
- Deleted messages can still be visible to recipients who saw them before deletion.
