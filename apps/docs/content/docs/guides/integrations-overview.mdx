...
Some integrations require specific license tiers:

- **Webhooks**: Available on all tiers
- **SocketClient**: Available on all tiers
- **Plugins**: Available on all tiers
- **MCP**: Available on all tiers
- **Chatwoot**: Check current licensing requirements
- **Cloudflare Proxy**: Check current licensing requirements

Check the [pricing page](/docs/licensing/pricing) for current tier details.
...
1. Run open-wa with `--webhook` flag
2. Point it at your service endpoint
...
1. Run open-wa with `--mcp` flag
2. Point your AI client at the MCP endpoint
3. Let the agent discover and call WhatsApp methods
