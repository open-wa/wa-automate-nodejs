...
Licensing follows the methods and runtime behavior that an integration uses, not only its wrapper name. Each integration can call different parts of the API surface. Examine the [pricing page](/docs/licensing/pricing) and method reference before you promise a workflow to a customer.
...
4. If your build does not mount `/mcp`, give the agent the HTTP API docs or OpenAPI schema. You can also give it the Postman collection.
