...
Use these sends when you want your agent to offer fixed choices instead of free-form text.
...
Install the external queue packages in your application before you use this skeleton:
...
Expected behavior: The bot receives text messages. It ignores group messages that do not mention `@bot`. Two queue workers limit concurrent LLM jobs. The limiter controls the interval between WhatsApp replies. Logs show each outgoing response when it leaves the limiter.
