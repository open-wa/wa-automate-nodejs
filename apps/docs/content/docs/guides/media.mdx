...
A data URL gives the runtime the file bytes and the MIME type in one string. Use this shape when sending media from disk, a buffer, or another service:
...
If a send fails or has the incorrect type, examine the data URL prefix first. Common problems include a missing `;base64` value or an `image/jpg` value instead of `image/jpeg`. A WebP image can fail in a non-sticker helper. A remote URL can return an auth page with `text/html` instead of the file.
...
Use signed URLs when possible. They do not put long-lived credentials in the browser or runtime layer. If you use headers, make sure that the endpoint returns the file bytes and a correct `Content-Type` header.
...
| Documents | WhatsApp Web can accept larger files, but keep automation payloads under 100 MB unless you have tested your account, browser, and host memory |
...
For production flows, decrypt and store media when you receive it. If the media is necessary for compliance, support, or analytics, save it before you complete the workflow. Use disk, object storage, or the S3 integration.
