...
| `--webhook "https://..."`, `-w "https://..."` | `webhook` | Webhook target URL for event delivery. |
...
| `--mcp` | `mcp.enabled` equivalent | Enables the MCP server for AI-agent clients in docs-facing examples. If your v5 alpha build does not parse this flag, use `mcp: { enabled: true }` in config. Keep `apiKey` enabled with MCP. |
| `--api-host "https://api.example.com"` | API host metadata | Public API host value used by docs-facing examples when the public address differs from the local bind address. It is not currently a core `ConfigSchema` field. |
...
npx @open-wa/wa-automate \
  ...
  --license-key "$OPENWA_LICENSE_KEY" \
  --link-code "447123456789"
```
...
Not every flag is a schema field. `--config` selects which file to load, `--pm2` controls process management, `--name` controls the PM2 process name, and v5-alpha docs-facing flags such as `--api-host` may not map to a core schema field.
...
| `--mcp` | Used in MCP docs as the shorthand for `mcp.enabled`. If your installed v5 alpha does not parse it, configure `mcp: { enabled: true }` in `wa.config.*`. |
| `--api-host` | Used in some docs as public-host metadata. It is not currently a core config schema field. |
