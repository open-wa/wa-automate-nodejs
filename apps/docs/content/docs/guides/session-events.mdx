...
4. `client.ready`, the client can send commands and receive messages.
...
QR codes and session data are sensitive auth material. Anyone who can read them can be able to connect or restore a WhatsApp session.

Keep these rules strict:

- do not log QR strings, `sessionData`, or base64 session payloads.
- do not send them to analytics, crash reporting, or shared support channels.
...
- Replace or delete stored session data when a device is lost, an employee leaves, or a session is no longer trusted.
...
Use `ev` for process-level lifecycle events. It can see startup, authentication, QR generation, disconnects, session data, and other runtime signals that can happen before a client is ready.
...
- `client.onMessage` and similar listeners give WhatsApp traffic data. For example, they show the received message and the necessary bot response.
...
Wildcard listeners can capture sensitive data. Filter by namespace and redact the data before you log it. Do not use catch-all listeners in production without a clear retention policy.
