...
```bash
npx @open-wa/wa-automate@alpha --session-id dictation-test --port 8080
```

Send a voice note to your session. The plugin replies with the transcription. If it does not reply, make sure that the message is an OGG voice note. Also make sure that `STT_API_KEY` is set.
...
- **Empty transcription**: Still sent (the user can want to know the API heard nothing)
