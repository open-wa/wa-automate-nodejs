...
**Performance note:** This interceptor runs for every message send. Keep it fast so that it does not add latency.
...
Register tools that AI agents or automation scripts can call. Keep tool descriptions narrow and validate arguments. Do not expose actions that an agent must not run.
