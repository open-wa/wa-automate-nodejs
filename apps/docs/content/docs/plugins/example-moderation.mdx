...
This example makes a plugin that scans incoming group messages with the OpenAI Moderation API. The plugin acts on flagged content. It can detect, label, or report the content. When permitted, it can delete the content or stop downstream handling.
...
- There can be a time limit on how old a message can be deleted
- Incoming messages can have different deletion behavior than sent messages
