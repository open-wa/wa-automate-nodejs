...
Plugins cannot read or write arbitrary files on the host system. Limit file operations to the data directory of the plugin.
...
- **Throw** — For unrecoverable errors (invalid config detected at runtime, critical dependency failure). This can crash the plugin.
- **`dispose()` and unregister** — Use this option for errors that must stop the plugin, such as an expired license or a revoked API key.
