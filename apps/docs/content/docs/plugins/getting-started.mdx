...
description: Build a small plugin that listens for a message, checks necessary fields, and sends a reply.
---
...
Put the plugin code from Step 2 in `plugins/greeting-bot.mjs`. The plugin loader uses dynamic `import(ref)`, so the local plugin must contain loadable JavaScript. Do not use `./plugins/greeting-bot.ts` unless your Node.js environment installs a TypeScript loader. The plugin loader does not compile TypeScript.
...
```bash
npx @open-wa/wa-automate@alpha --config ./wa.config.mjs --port 8080
```

If this Easy API instance is reachable outside your machine, add `--api-key "your-secure-key"` so plugin-mounted routes and the primary API share the same authentication boundary.

The expected local startup logs are examples, not captured output. A successful load includes plugin loader and registration messages similar to these messages:
...
The expected plugin failure logs are shortened examples. A missing file or incorrect config stops the load before the plugin handles messages. The log shows a message similar to this message:
...
### If a hook never fires

- **The event is filtered.** Plugins only receive public events. Subscribing to
  internal or sensitive events (`launch.*`, `browser.*`, `transport.*`,
  `license.*`) via the catch-all `event` hook will silently receive nothing —
  this is enforced by the plugin event gateway, see **[Security model](/docs/plugins/security-model)**.
- **Wrong hook name.** Hook keys are exact strings (for example
  `message.received`, `client.ready`). A typo registers a hook that never
  matches. Cross-check against the **[Hooks reference](/docs/plugins/hooks-reference)**.
- **The plugin did not actually register.** Confirm `plugin_registered` appears
  in the logs before assuming the hook is the problem.

### Duplicate plugin name

If two loaded plugins share the same `meta.name`, registration **throws**
`Plugin "<name>" is already registered` and startup fails — it is not silently
skipped. Give every plugin a globally unique `meta.name`.
