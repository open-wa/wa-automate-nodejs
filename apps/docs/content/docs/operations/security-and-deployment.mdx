...
description: Protect your API, manage keys, and deploy to production safely.
---
...
For monorepo development and source builds, the repo declares **Node.js >= 22.21.1** and pins `packageManager` to **pnpm@11.9.0**. Match those versions before debugging runtime or build issues.
...
npx @open-wa/wa-automate@alpha --port 8080 --api-key "$WA_API_KEY"
```
...
If your webhook endpoint is public, implement signature verification to make sure requests come from your open-wa instance.
...
- Simplest setup
- Run with `npx @open-wa/wa-automate@alpha --port 8080 --api-key "key"`
...
- **[Cloudflare Session Proxy](/docs/client-and-integrations/cf-proxy)**: Protected remote access
