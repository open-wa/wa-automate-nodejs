...
```bash
export OPENWA_API_KEY="your-generated-key"
npx @open-wa/wa-automate --port 8080 --api-key "$OPENWA_API_KEY"
```

Or in a `.env` file:

```
OPENWA_API_KEY=your-generated-key
```
...
services:
  sales-bot:
    ...
    ports:
      - "8080:8080"
    volumes:
      - ./sessions/sales:/app/sessions
    environment:
      - OPENWA_API_KEY=sales-key
    command: --session-id sales --port 8080
  ...
  support-bot:
    ...
    ports:
      - "8081:8081"
    volumes:
      - ./sessions/support:/app/sessions
    environment:
      - OPENWA_API_KEY=support-key
    command: --session-id support --port 8081
```
