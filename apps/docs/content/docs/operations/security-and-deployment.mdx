...
For monorepo development and source builds, the repo declares **Node.js >= 22.21.1** and **pnpm >= 11.9.0**, with `packageManager` pinned to **pnpm@11.9.0**. Match those versions before debugging runtime or build issues.
