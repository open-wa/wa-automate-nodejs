...
export async function create(
  ...
  try {
    ...
    const WA_AUTOMATE_VERSION = `${pkg.version}${
    ...
    if (VALID_SESSION) {
      ...
      if (config?.restartOnCrash)
        waPage.on('error', async (error) => {
          ...
          await kill(waPage);
          await create(sessionId, config, customUserAgent).then(
       ...
       */
      const BROKEN_METHODS = config?.skipBrokenMethodsCheck
        ? []
        : await waPage.evaluate(
            (checkList) => {
            ...
            uniq(
              fs
                .readFileSync(path.join(__dirname, '../lib', 'wapi.js'), 'utf8')
                .match(/(Store[.\w]*)\(/g)
                .map((x: string) => x.replace('(', ''))
            )
          );
      
      const LANG_CHECK = await waPage.evaluate(() => {
        //@ts-ignores
        if (window.l10n.localeStrings['en']) return (window.l10n.localeStrings['en'][0].findIndex((x) => x.toLowerCase() == 'use here') == 260);
        else return false;
      });
      ...
      if (!LANG_CHECK)
        console.log(
          'Some language based features (e.g forceRefocus) are broken. Please report this in Github.'
        );
      ...
      if (config?.licenseKey) {
        ...
        const { data } = await axios.post(pkg.licenseCheckUrl, {
          key: config.licenseKey,
          number: me._serialized,
          ...debugInfo,
        });
        if (data) {
