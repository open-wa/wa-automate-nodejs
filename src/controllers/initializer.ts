...
//export async function create(sessionId?: string, config?:ConfigObject, customUserAgent?:string) {
  ...
  const authenticatedKillerRace = [];
  if(config&&config.restoreKillTimer){
      authenticatedKillerRace.push(timeout(config.restoreKillTimer*1000))
  }
  authenticatedKillerRace.push(isAuthenticated(waPage))
  const authenticated = await Promise.race(authenticatedKillerRace);
  if(authenticated=='timeout') {
    console.log('Restore Session timed out. Shutting down')
    await kill(waPage);
    spinner.remove();
    throw new Error('Restore Session Timeout');
  }
