...
export class Whatsapp {
     ...
     * @param groupId group id    public async getGroupMembersId(groupId: string) {
      return await this.page.evaluate(
        groupId => WAPI.getGroupParticipantIDs(groupIReturns group members [Contact] objects
* @param groupId   public async getGroupMembers(groupId: string) {
           const membersIds = await this.getGroupMembersId(groupId);
           const actions = membersIds.map(memberId => {
this.getContact(memberId._serialized);
        return await Promise.all(act}

    ic async getContact(contactId: string) {
           return await this.page.evaluate(
          contactId =>       WAPI.getContact(contactI          * Retrieves chat object of given contact id
          * @param contactId
          * @returns contact detial as pr*/
    ic async getChatById(contactId: string) {
           return await this.page.evaluate(
